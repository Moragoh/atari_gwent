...
#For everyone
placedWeather = []
dupe_placed_weather = placedWeather
damageTotal = 0
damageTotalE = 0
turn_number = 0
...
"""
for i in range(3): 
  placedCards.append([''] *  8) 
          #card.damage
"""

"""
def askForFaction():   
  askFaction = input("What Faction would you like to be? (nilfgaard, nothern realms, scoia'tael, monsters) (IN ALL LOWERCASES, PLEASE.)\n")
  global playerFaction  
  if askFaction == 'nilfgaard':
    playerFaction = FACTIONS[0]
    #chosenDeck =   
  elif askFaction == 'nothern realms': 
    playerFaction = FACTIONS[1]
  elif askFaction == "scoia'tael":   
    playerFaction = FACTIONS[2]
  elif askFaction == 'monsters':   
    playerFaction = FACTIONS[3] 
  else: 
    print('Looks like your input was inccorect, please try again\n')
    askForFaction()
...
#WORKING
def getHand():
  playerDeck = north_deck #northDeck
  enemyDeck = nilfgaard_deck #GaardDeck 
  maxNum = len(playerDeck)
  maxNum_2 = len(enemyDeck) 
...
def placeCard():
  ...
  if pickedNum == 'pass':
    ...
    pickedInt = int(pickedNum)

    for card in playerHand:
      cardCheck.append(card.drawNum)

    if pickedInt in cardCheck:
      for card in north_deck:
        if card.drawNum == pickedInt:
          if card.row == 1:
            placedCards.append(card)
            placedMelee.append(card) 
...
def placeCard_AI():
  ...
  global enemy_pass
  dupe_enemy_placed = enemyPlaced
  dupe_placed = placedCards
  ...
  if turn_number > 6: 
    def weather_use():
      ...
      def check_frost():
        ...
        global enemys_placed_cards
        #Check for frost
        if Frost_30 in enemyHand:
          dupe_enemy_placed.append(Frost_30)
          dupe_placed_weather.append(Frost_30)

          if dupe_damage_totale > dupe_damage_total:
            placedWeather.append(Frost_30)
            enemyHand.remove(Frost_30)
            enemys_placed_cards += 1
            turn_number = turn_number + 1
            turn = 'player'

          else:
            only_weather = any(card in not_weather_north for card in enemyHand)
        ...
        elif Frost_31 in enemyHand:
          dupe_enemy_placed.append(Frost_31)
          dupe_placed_weather.append(Frost_31)

          if dupe_damage_totale > dupe_damage_total:
            placedWeather.append(Frost_31)
            enemyHand.remove(Frost_31)
            enemys_placed_cards += 1
            turn_number = turn_number + 1
            turn = 'player'
      ...
      def check_fog():
        ...
        global enemys_placed_cards
        #Check for Fog
        if Fog_31 in enemyHand:
          dupe_enemy_placed.append(Fog_31)
          dupe_placed_weather.append(Fog_31)
 
          if dupe_damage_totale > dupe_damage_total:
            placedWeather.append(Fog_31)
            enemyHand.remove(Fog_31)
            enemys_placed_cards += 1
            turn_number = turn_number + 1
            turn = 'player'
        ...
        elif Fog_32 in enemyHand:
          dupe_enemy_placed.append(Fog_32)
          dupe_placed_weather.append(Fog_32)

          if dupe_damage_totale > dupe_damage_total:
            placedWeather.append(Fog_32)
            enemyHand.remove(Fog_32)
            enemys_placed_cards += 1
            turn_number = turn_number + 1
            turn = 'player'
      ...
      def check_rain():
        ...
        global enemys_placed_cards
        #Check for weather
        if Rain_32 in enemyHand:
          dupe_enemy_placed.append(Rain_32)
          dupe_placed_weather.append(Rain_32)

          if dupe_damage_totale > dupe_damage_total:
            placedWeather.append(Rain_32)
            enemyHand.remove(Rain_32)
            enemys_placed_cards += 1
            turn_number = turn_number + 1
            turn = 'player'
        ...
        elif Rain_33 in enemyHand:
          dupe_enemy_placed.append(Rain_33)
          dupe_placed_weather.append(Rain_33)

          if dupe_damage_totale > dupe_damage_total:
            placedWeather.append(Rain_33)
            enemyHand.remove(Rain_33)
            enemys_placed_cards += 1
            turn_number = turn_number + 1
            turn = 'player'
      ...
      def check_sunny():
        ...
        global turn_number
        global enemys_placed_cards
        if ClearWeather_33 in enemyHand:
          dupe_enemy_placed.append(ClearWeather_33)
          dupe_placed_weather.append(ClearWeather_33)

          if dupe_damage_totale > dupe_damage_total:
            placedWeather.append(ClearWeather_33)
            enemyHand.remove(ClearWeather_33)
            enemys_placed_cards += 1
            turn_number = turn_number + 1
            turn = 'player'
        ...
        elif ClearWeather_34 in enemyHand:
          dupe_enemy_placed.append(ClearWeather_34)
          dupe_placed_weather.append(ClearWeather_34)

          if dupe_damage_totale > dupe_damage_total:
            placedWeather.append(ClearWeather_34)
            enemyHand.remove(ClearWeather_34)
            enemys_placed_cards += 1
            turn_number = turn_number + 1
            turn = 'player'
  ...
  #If using weather cards aren't beneficial
    ...
    if enemys_placed_cards < 11:
      if enemy_index == 11:
        ...
        #Check if melee
        if enemy_card.row == 1:
          enemyPlaced.append(enemy_card)
          enemyMelee.append(enemy_card)
          enemys_placed_cards += 1
          turn = 'player'
        #Check if ranged
        elif enemy_card.row == 2:
          enemyPlaced.append(enemy_card)
          enemyRanged.append(enemy_card)
          enemys_placed_cards += 1
          turn = 'player'
        #Check if siege
        elif enemy_card.row == 3:
          enemyPlaced.append(enemy_card)
          enemySiege.append(enemy_card)
          enemys_placed_cards += 1
          turn = 'player'
...
def gwentBoard(): 
  ...
  #Check for frost
  if Frost_30 in placedWeather:
    weather_melee = 1
  elif Frost_31 in placedWeather:
    weather_melee = 1

  #Check for fog
  if Fog_31 in placedWeather:
    weather_ranged = 1
  elif Fog_32 in placedWeather:
    weather_ranged = 1

  #Check for rain
  if Rain_32 in placedWeather:
    weather_siege = 1
  elif Rain_33 in placedWeather:
    weather_siege = 1
  
  #Check for Clear Weather  
  if ClearWeather_33 in placedWeather: 
    for card in placedWeather:
      placedWeather.remove(card)
  elif ClearWeather_34 in placedWeather:
    for card in placedWeather:
      placedWeather.remove(card)
  ...
  #For the player
  if weather_melee == 1:
    for card in placedMelee:
      placedMeleeTotal = placedMeleeTotal + card.weatherDamage
      damageTotal = damageTotal + card.weatherDamage
  else:
    for card in placedMelee:
      placedMeleeTotal = placedMeleeTotal + card.damage
      damageTotal = damageTotal + card.damage

  if weather_ranged == 1:
    for card in placedRanged:
      placedRangedTotal = placedRangedTotal + card.weatherDamage
      damageTotal = damageTotal + card.weatherDamage  
  else:
    for card in placedRanged:
      placedRangedTotal = placedRangedTotal + card.damage
      damageTotal = damageTotal + card.damage

  if weather_siege == 1:
    for card in placedSiege:
      placedSiegeTotal = placedSiegeTotal + card.weatherDamage
      damageTotal = damageTotal + card.weatherDamage
  else:
    for card in placedSiege:
      placedSiegeTotal = placedSiegeTotal + card.damage
      damageTotal = damageTotal + card.damage
  ...
  #For the AI
  if weather_melee == 1:
    for card in enemyMelee:
      enemy_melee_total = enemy_melee_total + card.weatherDamage
      damageTotalE = damageTotalE + card.weatherDamage
  else:
    for card in enemyMelee:
      enemy_melee_total = enemy_melee_total + card.damage
      damageTotalE = damageTotalE + card.damage

  if weather_ranged == 1:
    for card in enemyRanged:
  ...
  for x in range (3):
    ...
    if x == 0:
      ...
      for card in placedMelee:
        cardName = card.nickName
        print (" [%s] " % (cardName), end = ' ')
      print('   ROW TOTAL: %s ' % (placedMeleeTotal), end = ' ') 
      print()
      print(SLINE)
      print(MIDLINE)
      ...
      for card in placedRanged:
        cardName = card.nickName
        print (" [%s] " % (cardName), end = ' ')
      print('   ROW TOTAL: %s ' % (placedRangedTotal), end = ' ') 
      print()
      print(SLINE)
      print(MIDLINE)
      ...
      for card in placedSiege:
        cardName = card.nickName
        print (" [%s] " % (cardName), end = ' ')
      print('   ROW TOTAL: %s ' % (placedSiegeTotal), end = ' ')  
      print()
      print(SLINE)
      print(MIDLINE)
...
while True:
  ...
  while game_on:
    #Setting the game up
    dupe_board()
    player_cards = len(playerHand)
    enemy_cards = len(enemyHand)
    ...
    if turn == 'player':
      if 'yes' in player_pass:
        ...
        if 'yes' in enemy_pass:
          if damageTotal > damageTotalE:
            print()
            print('You win!')
            game_on = False
          if damageTotalE > damageTotal:
            print()
            print('You lose!')
            game_on = False
          elif damageTotal == damageTotalE:
            print()
